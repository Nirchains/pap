...
foreach ($origData as $key => $value) {
	...
	//tutelas
	if (!in_array($value->t_solicitudes_tutelas___tutelas_raw, $origDataTutelasArrId)) {
		array_push($origDataTutelasArrId, $value->t_solicitudes_tutelas___tutelas_raw);
		array_push($origDataTutelasArr, $value);
	}	
}
...
foreach ($newData as $key => $value) {
	...
	//tutelas
	if (!in_array($value->t_solicitudes_tutelas___tutelas_raw, $newDataTutelasArrId)) {
		array_push($newDataTutelasArrId, $value->t_solicitudes_tutelas___tutelas_raw);
		array_push($newDataTutelasArr, $value);
	}	
}
...
<?php
	...
	foreach ($origDataTutelasArr as $value) {
		...
		echo "<td>" . $value->t_solicitudes_tutelas___asignaturas . "</td>";
		echo "<td>" . $value->t_solicitudes_tutelas___tutelas . "</td>";
	...
	foreach ($newDataTutelasArr as $value) {
		//echo "<br><br>--------------------------------------------<br><br>";
		//echo json_encode($value) . " <br>***---***<br> " . json_encode($origDataTutelasArr);
		$ret = estadoTutela($value, $origDataTutelasArr);
...
function estadoTutela($asignatura, $arrbuscar) {
	foreach ($arrbuscar as $key => $value) {
		if ($value->t_solicitudes_tutelas___tutelas_raw == $asignatura->t_solicitudes_tutelas___tutelas_raw) { 
			if ($value->t_solicitudes_tutelas___creditos_asignados != $asignatura->t_solicitudes_tutelas___creditos_asignados) {
				//datos modificados
				return 1;
			} else {
				return 0;
